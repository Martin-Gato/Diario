...
const ENTRIES = [
  ...
  {
    id: "cierre-dashboard-operaciones-powerbi-04",
    dateStart: "2026-09-11",
    dateEnd: "2026-09-18",
    project: "Cierre del Dashboard de Operaciones: Nuevas Páginas y Corrección de Bugs de Fondo (Power BI)",
    tags: [
      "Power BI",
      "DAX",
      "Power Query",
    ...
    summary:
      "Continué y cerré el proyecto del dashboard de operaciones iniciado la semana anterior. A lo largo de varias sesiones construí dos páginas nuevas, rediseñé varias veces la lógica de negocio que había reconstruido por evidencia hasta encontrar versiones que no perdieran ni corrompieran datos, encontré y corregí 6 bugs de fondo preexistentes en el modelo, y cerré con un documento final que consolida todo el trabajo (hallazgos, código en producción y pendientes) para quien retome el proyecto.",
    tasks: [
      "Construí dos páginas nuevas en el dashboard: una de auditoría de errores accionable sobre el ciclo completo de un pedido (5 fechas clave e indicadores), y otra de tendencias históricas sobre pedidos ya completados y archivados.",
      "Rediseñé en 3 iteraciones sucesivas la consulta que captura la fecha de surtido interno de cada pedido, hasta llegar a una versión que no perdiera ni corrompiera fechas por el contenido de un mensaje de error.",
      "Rediseñé en 4 iteraciones la lógica que captura la fecha real de facturación, evaluando y descartando varias fuentes hasta encontrar una que fuera consistente y tuviera hora real (no solo fecha).",
      "Investigué a fondo un supuesto hallazgo mayor (un evento del proceso pareciendo ocurrir sistemáticamente después de otro que debería sucederlo), descartando 6 hipótesis de negocio antes de encontrar la causa real y retractar el hallazgo.",
      "Diseñé y validé con 7 pruebas cruzadas sobre datos reales el criterio que distingue pedidos 'vivos' de 'históricos' en el modelo, antes de construir las páginas que dependen de ese criterio.",
      "Documenté 5 casos de estudio individuales que ilustran patrones de error distintos encontrados durante la auditoría (un estatus interno desincronizado tras un error de otro sistema, eventos que nunca quedan registrados en el log de auditoría, mensajes de error confundidos con ruido, contención real del sistema, y pedidos que no se archivan pese a estar completos).",
      "Detecté un patrón operativo (backlog de pedidos concentrado de forma desproporcionada en un solo responsable) que era invisible en las gráficas existentes porque excluían automáticamente los pedidos sin cierta fecha, y agregué una gráfica nueva para exponerlo.",
      "Redacté el documento final del proyecto: consolida las seis versiones de hallazgos generadas durante todas las sesiones, el código final en producción, la cronología de decisiones de diseño y los pendientes abiertos para quien retome el trabajo.",
    ],
    findings: [
      "Antes de concluir que dos eventos de un proceso ocurren en el orden incorrecto, conviene buscar primero un desfase de zona horaria sin corregir: la desviación estándar del desfase distingue un bug de huso horario (se mide en segundos) de variabilidad real de negocio (se mide en días).",
      "Un filtro que excluye registros por el contenido de un campo de texto puede borrar también otras columnas válidas de esa misma fila (como la fecha) si se aplica en la cláusula de filtrado principal de la consulta; es más seguro mover esa lógica de exclusión a una expresión condicional dentro de la selección de columnas.",
      "Cuando dos columnas de una misma fila deben anclarse a eventos distintos de una misma serie (ej. fecha del primer evento, mensaje del más reciente), un auto-cruce de la misma consulta contra sí misma, numerando filas en ambos sentidos, resuelve el problema sin duplicar la lógica de extracción.",
      "En DAX, una fórmula que depende del contexto de la fila actual (como una búsqueda de valor sin relación de modelo) solo funciona como columna calculada, nunca como medida; es la alternativa correcta cuando crear una relación en el modelo generaría un ciclo irresoluble.",
      "Antes de asumir que un dashboard tiene un error, vale la pena validar si el sistema fuente es el que tiene el dato desactualizado: un caso real mostró que un problema de inventario a nivel de artículo dejaba un estatus de pedido 'huérfano' en el sistema origen, aunque el proceso de negocio ya había avanzado por otra vía.",
      "Reducir la concurrencia de consultas configurada globalmente en la herramienta de BI (no solo en el archivo) mitigó los cortes de conexión por saturación del servidor durante la actualización de datos, a costa de un refresh más lento.",
    ],
    pending: [
      "Trasladar a quien administra la interfaz personalizada las preguntas de negocio que quedaron abiertas (qué reemplazó a un estatus descontinuado, y el significado exacto de ciertos campos y eventos).",
      "Solicitar a TI la creación de un índice recomendado sobre la tabla de log de cambios, que hoy hace lentas las consultas que dependen de ella.",
      "Diseñar y aplicar al proceso de facturación una columna de auditoría equivalente a la ya construida para el proceso de surtido.",
      "Evaluar separar el reporte en uno operativo y uno de auditoría, para desacoplar el refresh pesado de las páginas de uso diario.",
      "Actualizar antes de inicios de 2027 la ventana de fechas fija usada por una de las consultas, o migrarla a una ventana rodante.",
      "Corregir el bug ya documentado (pendiente desde la sesión anterior) de la página que sigue usando la tabla incorrecta para calcular tiempos.",
      "Reportar a quien administra el ERP un registro con una fecha inválida detectado durante la validación de datos.",
    ],
  },
];
